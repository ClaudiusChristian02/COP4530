...
namespace fsu
{
	...
	template <class I, class P>
	void g_heap_repair(I beg, size_t i, I end, P & pred)
		// the default order version
	{
		if (end - beg < 2)
			return;

		// corrected an off by one removed minus 1
		size_t n = end - beg;
		size_t l, r;
		bool finished = 0;
		do
	...
	} // end g_heap_repair(I beg, size_t i, I end)
		...
		//         the end of the old range (i.e., to beg[end - beg - 1])
	{
		...
		g_heap_repair(beg, 0, end - 1, pred);

	} // end g_pop_heap()

	template <class I>
	void g_heap_repair(I beg, size_t i, I end)
		// the default order version
	{
		if (end - beg < 2)
			return;
	...
	void g_build_heap(I beg, I end)
	{
		...
		for (size_t i = n / 2; i > 0; --i) {
			fsu::g_heap_repair(beg, i, end);
		}

		fsu::g_heap_repair(beg, 0, end);
	}
	...
	void g_build_heap(I beg, I end, P & pred)
	{
		size_t n = end - beg;
		for (size_t i = n / 2; i > 0; --i) {
			fsu::g_heap_repair(beg, i, end, pred);
		}

		fsu::g_heap_repair(beg, 0, end, pred);
	}
...
namespace alt
{
	...
	void g_build_heap(I beg, I end)
	{
		...
		for (auto i = beg + 1; i != end; ++i) {
			fsu::g_push_heap(beg, i);
		}

		fsu::g_push_heap(beg, end);
	}
...
template<typename I> void g_build_heap(I beg, I end)
{
size_t n = end - beg;
for (size_t i = n / 2; i > 0; --i) {
fsu::g_heap_repair(beg, beg + i, end);
}

fsu::g_heap_repair(beg, beg, end);
}
