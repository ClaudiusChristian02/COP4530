/*
    ...
    <file documentation>
	<Chris Lacher created all of the file contents except for
	Linkin, LinkedOut, and reverse functions>
*/

/* 
    Instructions:
      1. Rename this file to "list.cpp"
      2. Supply implementions where missing (search for "// TBS")
      3. Supply correct file header
      4. Delete these instructions

      requiring implementation: Reverse, LinkIn, LinkOut

      NOTE: When a TBS method requires a return value, the supplied code creates
      a return only to satisfy the syntactic requirements so that the code will
      build without complaint. These are not intended to be a hint and in most
      cases will require erasure.
...
{
  // TBS
  // use only two local variables - suggest these:
  // Link * link;
  // Link * temp;
	...
	Link * link = tail_;                      // to remember the last element
	
	while (head_->next_ != link){              // read through list until the last node
		Link * temp = LinkOut(head_->next_);  // moving first element and point head to next
		LinkIn(link, temp);                   // moves the first element to the end
		link = temp;                          // moves link pointer to temp node

	} // end while (head->next_ != link){
} // end void List<T>::Reverse ()
...
// link newLink into list at (ahead of) location
{
  ...
  typename List<T>::Link *previous = location->prev_;
  previous->next_ = newLink;
  newLink->next_ = location;
  newLink->prev_ = previous;
...
{
  // TBS
	typename List<T>::Link *previous = oldLink->prev_;
	typename List<T>::Link *next = oldLink->next_;
	previous->next_ = next;
	next->prev_ = previous;
