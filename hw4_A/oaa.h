...
namespace fsu
{
	...
	private: // definitions and relationships
		...
		class Node
		{
			...
			bool LeftChildIsRed() const {
				return lchild_ != nullptr && lchild_->IsRed(); }
			bool RightChildIsRed() const {
				return rchild_ != nullptr && rchild_->IsRed(); }
		};
		...
		// recursive left-leaning get
	{
		...
		{
			location = NewNode(kval, DataType());
			return location;
		}
		if (this->pred_(kval, nptr->key_))       // left subtree
		{
			nptr->lchild_ = RGet(nptr->lchild_, kval, location);
		}
		else if (this->pred_(nptr->key_, kval))  // right subtree
		{
		...
		// recursive left-leaning insert
	{
		...
		if (pred_(key, nptr->key_))       // left subtree is not null
		{
			nptr->lchild_ = RInsert(nptr->lchild_, key, data);
		}
		else if (pred_(nptr->key_, key))  // right subtree
		{
		...
		if (nptr->rchild_->IsRed() && !nptr->lchild_->IsRed())
			nptr = RotateLeft(nptr);

		if (nptr->lchild_->IsRed() && nptr->lchild_->lchild_->IsRed())
			nptr = RotateRight(nptr);

		if (nptr->lchild_->IsRed() && nptr->rchild_->IsRed())
